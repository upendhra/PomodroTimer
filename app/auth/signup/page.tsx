...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // Get the active session to ensure auth context is active for RLS
        ...
        if (!sessionData.session) {
          throw new Error("Unable to establish session. Please check your email for confirmation.");
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-indigo-50 to-purple-50 text-gray-900 overflow-hidden relative">
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-white/60 backdrop-blur-md px-4 py-2 rounded-full shadow-md">
        <span className="text-sm text-gray-600">Looking for the main site?</span>
        <Link href="/" className="text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-cyan-500 hover:opacity-80 transition-opacity">
          Home
        </Link>
      </div>
